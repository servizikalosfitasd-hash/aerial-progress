...
export const SkillCard = ({ skill, currentProgressionName, progressPercent, index = 0 }: SkillCardProps) => {
  return (
    <Link
      to={`/skill/${skill.id}`}
    ...
    >
      ...
      <div className="relative aspect-[4/5] overflow-hidden">
        ...
        <div className="absolute top-4 left-4 right-4 flex items-start justify-between">
          <span className={`text-[10px] font-semibold tracking-wider uppercase px-2.5 py-1 rounded-full border backdrop-blur-md ${difficultyColor[skill.difficulty]}`}>
            {skill.difficulty}
          </span>
          <div className="h-9 w-9 rounded-full bg-background/40 backdrop-blur-md border border-border/50 flex items-center justify-center transition-all duration-300 group-hover:bg-primary group-hover:border-primary group-hover:rotate-45">
            <ArrowUpRight className="h-4 w-4 text-foreground transition-colors group-hover:text-primary-foreground" strokeWidth={2.5} />
          </div>
        </div>
